...
vec4 swayRandomized(vec4 seed, vec4 value)
{
    vec4 f = floor(value);
    vec4 start = sin((cos(f * seed) + sin(f * 1024.)) * 345. + seed);
    vec4 end   = sin((cos((f+1.) * seed) + sin((f+1.) * 1024.)) * 345. + seed);
