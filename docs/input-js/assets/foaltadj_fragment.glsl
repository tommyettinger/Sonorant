...
float hash(float seed, float value) {
    float f = floor(value);
...
float foam(float seed, vec4 i) {
    ...
    return smoothstep(0.0, 1.0, smoothstep(0.0, 1.0, (a + b + c + d + e) * 0.1 + 0.5));
}
...
void main() {
    ...
    con.x = foam(u_seed, con);
    con.y = foam(u_seed, con);
    con.z = foam(u_seed, con);
