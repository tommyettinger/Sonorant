 ...
 */
public class EndlessChanger extends ApplicationAdapter {

    private SpriteBatch batch;
    private Texture pixel;
    private int shaderIndex = 0;
    private final ShaderProgram[] shaders = new ShaderProgram[1];
    ...
    public EndlessChanger(Clipboard clippy, long initialSeed) {
        clipboard = clippy;
        reseed(initialSeed);
    }
    ...
    @Override public void create () {
        ...
        pixel = new Texture("pets.jpg");
        startTime = TimeUtils.millis();

        ShaderProgram.pedantic = true;
        ShaderProgram shaderHueChange;
        shaders[0] = shaderHueChange = new ShaderProgram(Gdx.files.internal("foam_vertex.glsl"), Gdx.files.internal("huechange_fragment.glsl"));
        if (!shaderHueChange.isCompiled()) {
            Gdx.app.error("Shader", "error compiling shaderHueChange:\n" + shaderHueChange.getLog());
            Gdx.app.exit();
            return;
        }
...
//		long state = System.nanoTime() + startTime;//-1234567890L;
    ...
    @Override public void resize (int width, int height) {
        EndlessChanger.width = width;
        EndlessChanger.height = height;
