 ...
 */
public class EndlessShaderNoise extends ApplicationAdapter {

    private SpriteBatch batch;
    private Texture pixel;
    private int shaderIndex = 0;
    private final ShaderProgram[] shaders = new ShaderProgram[18];
    ...
    @Override public void create () {
        ...
        if (!shaderBwAdj.isCompiled()) {
            Gdx.app.error("Shader", "error compiling shaderBwAdj:\n" + shaderBwAdj.getLog());
            Gdx.app.exit();
            return;
        }

        ShaderProgram shaderBananarAdj;
        shaders[8] = shaderBananarAdj = new ShaderProgram(Gdx.files.internal("foam_vertex.glsl"), Gdx.files.internal("bananaradj_fragment.glsl"));
        if (!shaderBananarAdj.isCompiled()) {
            Gdx.app.error("Shader", "error compiling shaderBananarAdj:\n" + shaderBananarAdj.getLog());
            Gdx.app.exit();
            return;
        }

        ShaderProgram shaderGrAdj;
        ...
        ShaderProgram shaderFoomAdj;
        shaders[15] = shaderFoomAdj = new ShaderProgram(Gdx.files.internal("foam_vertex.glsl"), Gdx.files.internal("foomadj_fragment.glsl"));
        if (!shaderFoomAdj.isCompiled()) {
            Gdx.app.error("Shader", "error compiling shaderFoomAdj:\n" + shaderFoomAdj.getLog());
            Gdx.app.exit();
            return;
        }

        ShaderProgram shaderValtueAdj;
        shaders[16] = shaderValtueAdj = new ShaderProgram(Gdx.files.internal("foam_vertex.glsl"), Gdx.files.internal("valtueadj_fragment.glsl"));
        if (!shaderValtueAdj.isCompiled()) {
            Gdx.app.error("Shader", "error compiling shaderValtueAdj:\n" + shaderValtueAdj.getLog());
            Gdx.app.exit();
            return;
        }

        ShaderProgram shaderFoaltAdj;
        shaders[17] = shaderFoaltAdj = new ShaderProgram(Gdx.files.internal("foam_vertex.glsl"), Gdx.files.internal("foaltadj_fragment.glsl"));
        if (!shaderFoaltAdj.isCompiled()) {
            Gdx.app.error("Shader", "error compiling shaderFoaltAdj:\n" + shaderFoaltAdj.getLog());
            Gdx.app.exit();
            return;
        }
...
//		long state = System.nanoTime() + startTime;//-1234567890L;
    ...
    @Override public void resize (int width, int height) {
        EndlessShaderNoise.width = width;
        EndlessShaderNoise.height = height;
